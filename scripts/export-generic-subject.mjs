// Regenerate data/{code}-snapshot.js + data/{code}-audience-snapshot.js for
// ANY subject_id that has real data in l1_silver.mention, using the generic
// (non subject-suffixed) l2_gold tables — agg_opportunity_generic,
// agg_risk_radar_generic, agg_audience, agg_platform. Unlike AJD/KLK (which
// have their own dedicated _ajd/_klk gold tables from an earlier pipeline),
// every other subject only has these generic tables, so this script is meant
// to be reused for any of them (ARR, BHL, KLT, KLU, KNU, ...).
//
// Run locally with: node scripts/export-generic-subject.mjs <CODE> <"Nama Lengkap"> ["Jabatan"]
// Example: node scripts/export-generic-subject.mjs ARR "Ahmad Rizal Ramdhani" "Direktur Utama Bulog"
// Requires DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD in .env.local.
...
async function writeSnapshot(filename, exports) {
  const body = Object.entries(exports)
    ...
    .join("\n\n");
  const content = `// Auto-generated by scripts/export-generic-subject.mjs — jangan edit manual.\n// Jalankan ulang skrip ini untuk regenerate.\n\n${body}\n`;
  await writeFile(path.join(DATA_DIR, filename), content, "utf8");
  console.log(`Wrote data/${filename}`);
}

async function main() {
  const [code, fullName, title] = process.argv.slice(2);
  if (!code || !fullName) {
    console.error('Usage: node scripts/export-generic-subject.mjs <CODE> "Nama Lengkap" ["Jabatan"]');
    process.exit(1);
  }
  const subjectId = code.toUpperCase();
  ...
  const audienceSeg = await client.query(
    `SELECT author_segment, n_total FROM l2_gold.agg_audience WHERE subject_id=$1 ORDER BY n_total DESC`,
    [subjectId]
  );
  ...
  const platformData = platform.rows.map((r) => ({ platform: cap(r.platform), value: Number(r.n) }));

  await writeSnapshot(`${lowerCode}-snapshot.js`, {
    snapshotMeta: {
      subjectId,
      subjectName: fullName,
      subjectTitle: title || null,
  ...
  // ---------- Audience / Community (derived from mention table only — no
  // real TikTok follower scrape exists for this subject, unlike AJD's) ----------
  const audienceSegDetail = await client.query(
  ...
  const emotion = await client.query(
    `SELECT emotion_label, count(*) n FROM l1_silver.mention
     WHERE subject_id=$1 AND attribution_layer IS NOT NULL AND emotion_label IS NOT NULL
     GROUP BY 1 ORDER BY 2 DESC`,
    [subjectId]
  );
  ...
  const relevantMentions = Number(ct.n_total);

  await writeSnapshot(`${lowerCode}-audience-snapshot.js`, {
    snapshotMeta: {
      subjectId,
      queryDate: new Date().toISOString().slice(0, 10),
      source: "kanalytics_spi_test (PostgreSQL) — l1_silver.mention (attribution_layer relevan)",
      missingFields: ["usia", "gender", "kota domisili", "data pengikut TikTok real (belum ada scrape Apify untuk figur ini)"],
    },
    kpi: {
