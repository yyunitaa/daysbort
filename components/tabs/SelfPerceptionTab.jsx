...
import { KPI, SectionLabel, Callout, cardCls, BLUE, GREEN, RED, GRAY, sentimentColor, chartAxisStyle, chartGridStroke, chartTooltipStyle, ChartTitle } from "../ui";
import {
  kpi, attributionData, platformData, audienceSegmentData,
  sentimentTrend, volumeTrend, topicEngagement, topTopicsVolume, riskRadar, topContent,
} from "../../data/ajd-snapshot";

export default function SelfPerceptionTab() {
  return (
    <div className="flex flex-col gap-10">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KPI icon={Users} label="Unggahan yang Benar Tentang Beliau" value={kpi.relevantMentions} sub={`Dari ${kpi.rawMentions.toLocaleString("id-ID")} unggahan yang ditemukan, sebagian besar ternyata bukan tentang beliau — nama "Amri" & "Jamaluddin" banyak dipakai orang lain.`} accent={BLUE} />
        <KPI icon={MessageCircle} label="Reaksi Warga yang Positif" value={`${kpi.positivePct}%`} sub="102 dari 157 unggahan yang benar-benar tentang beliau bernada positif." accent={GREEN} />
        <KPI icon={Activity} label="Skor Sentimen Keseluruhan" value={kpi.netSentiment} sub="Skala -100 (sangat negatif) sampai +100 (sangat positif) — angka ini tergolong baik." accent={GREEN} />
        <KPI icon={ShieldAlert} label="Isu yang Perlu Diperhatikan" value="1" sub={`${riskRadar.topicLabel} — kondisinya stabil minggu ini, tidak memburuk.`} accent={RED} />
      </div>
      ...
      <section>
        <SectionLabel eyebrow="4" title="Unggahan yang Paling Berpengaruh" caveat="Hanya unggahan dari akun resmi atau pernyataan langsung Bapak Amri — bukan dari akun yang belum tentu terkait." />
